...
Install the issuer authentication dependencies in `backend/` with `npm install`. Seed the development issuer after MongoDB is running:
...
The seed is idempotent and does not overwrite an existing account. The demo credentials are `issuer@authodox.demo` and `Demo@12345`. Sign in at `/issuer/login`; authenticated issuance is available at `/issuer/issue` and is protected by the backend JWT middleware.
